...
当前已建立第一版“日线 + 周线 + 60 分钟”结构摘要契约：

- 数据覆盖：判断日 K、周 K、60 分钟 K 是否足够。
- 趋势状态：用短中期均线和价格位置判断上升、震荡或下行。
- 中枢近似：基于近期 K 线区间生成观察用中枢上沿/下沿。
- 买点观察：输出二买候选、二买确认或无买点。
- 风险卖点：跌破近期结构防守位时优先输出风险卖点候选。
- 结论输入：结构摘要会生成 `decisionInput`，直接参与页面最终结论。

这还不是完整缠论实现。后续会继续补分型、笔、线段、中枢递归、背驰/盘整背驰、一买/三买和多级别联动。
